...
public class RelatoriosService {
    ...
    public ByteArrayInputStream gerarExcelRelatorioAcesso(List<Acesso> acessos) throws IOException {
    ...
    public ByteArrayInputStream gerarExcelRelatorioNotaExcel(List<NotaFiscalCompra> notaFiscalCompras) throws IOException {
        try (Workbook workbook = new XSSFWorkbook()) {
            Sheet sheet = workbook.createSheet("Acessos");
            ...
            Row headerRow = sheet.createRow(0);
            headerRow.createCell(0).setCellValue("Codigo");
            headerRow.createCell(1).setCellValue("dataCompra");
            // Adicione mais colunas, se necessário.

            // Preencha os dados dos clientes
            int rowNum = 1;
            for (NotaFiscalCompra notaFiscalCompra : notaFiscalCompras) {
                Row row = sheet.createRow(rowNum++);
                row.createCell(0).setCellValue(notaFiscalCompra.getId());
                row.createCell(1).setCellValue(notaFiscalCompra.getDataCompra());
                // Adicione mais colunas, se necessário.
            }
